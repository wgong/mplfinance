...
def _load_etf_df():
    WATCH_ETF = """
#####  watch
- SPY,QQQ,UNG,FCG,XLE,DBA,DBC,WEAT,KHC,TSN,TAP,GDX,SILJ,AXU,AU,EXK,XLB,XME,URA,EWA,ARGT
#####  index-sector
...
# Price must be in top panel
PANID_PRICE, PANID_VOL, PANID_RSI, PANID_SIGNAL = 0, 1, 2, 3
PANEL_RATIOS = (8, 1, 8, 1)
FIGURE_WIDTH, FIGURE_HEIGHT =  16, 10
YELLOW = '#F5D928'
LIGHT_BLACK = '#8F8E83'
...
def _gen_chart_v1(ticker, df, num=0, chart_root="images",
    ...
    rsi_plot = mpf.make_addplot(df["rsi"], panel=panid_rsi, color='k', width=1, #title=f"{ticker}-RSI",
                            fill_between=dict(y1=df["rsi_d"].values,y2=df["rsi_u"].values,alpha=0.15,color='b'),
                            ylim=(rsi_min,rsi_max))
    rsi_avg_plot = mpf.make_addplot(df["rsi_avg"], panel=panid_rsi, color="b", width=1, linestyle="solid",
                            ylim=(rsi_min,rsi_max))
    rsi_bar_plot = mpf.make_addplot(df["rsi"], panel=panid_rsi, type="bar", 
                            color=[_color_rsi_bar(v)  for v in df["rsi"]], 
                            ylim=(rsi_min,rsi_max))
    ...
    plots = [
            ...
            # panel-1
            , rsi_plot ,rsi_avg_plot, rsi_bar_plot   # , rsi_u_plot, rsi_d_plot 
            # panel-2
            , vol_avg_plot
            # panel-3
            , signal_plot
...
def do_gwg_chart():
    ...
    for ticker in selected_tickers:
        ...
        x = _gen_chart_v1(ticker, df)   # _gen_chart_v0(ticker, df)   # 
        st.image(x["file_img"])
